from _services.UploadNotifier.upload_notifier import UploadNotifier
from _services.circuits import store_circuit_data, update_circuit_data
from _services.drivers import (
    store_driver_data,
    store_driver_numbers,
    store_team_changes,
)
from _services.enums import (
    init_compounds,
    init_event_formats,
    init_season,
    init_session_types,
...
def store_session_data_to_db(
    season: str, round_number: int, session_type: int, is_store_session: bool = False
):
    if is_store_session:
    ...
    UploadNotifier().send_notification(
        year=season, event=str(round_number), session=str(session_type)
    )
...
def init_year_data(season: str):
    ...
    update_circuit_data(season=season)
